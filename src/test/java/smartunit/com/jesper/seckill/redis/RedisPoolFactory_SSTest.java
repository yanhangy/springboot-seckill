...
public class RedisPoolFactory_SSTest extends RedisPoolFactory_SSTest_scaffolding {
  ...
  public void test_JedisPoolFactory_0()  throws Throwable  {
      //caseID:c4b93015a79dfee26750e9e0b5e9fe8e
      ...
      doReturn("org.apache.catalina.realm.GenericPrincipal").when(redisConfig0).getHost();
      doReturn("=p").when(redisConfig0).getPassword();
      doReturn(1).when(redisConfig0).getPoolMaxIdle();
      doReturn(0).when(redisConfig0).getPoolMaxTotal();
      doReturn((-1)).when(redisConfig0).getPoolMaxWait();
      doReturn((-1)).when(redisConfig0).getPort();
      doReturn(1993).when(redisConfig0).getTimeout();
